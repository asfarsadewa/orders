...
- Design language: dark command post. `--bg #0c0e0d`, `--panel #141716`, `--ink #e6e8e3`, `--dim #8a9086`, `--rule #2a2e2a`; amber `#eaaa08` for command and warning, red `#f97066` critical, green `#75e0a7` acknowledged, blue `#84adff` note. IBM Plex Mono everywhere, Xanh Mono for the wordmark only. 21px rhythm, no rounded corners, no gradients, controls in a row share one box height. Dry copy, sentence case, as few words as possible.
