...
You command Vesper Station, a colony of 184 people, for 14 days after a systems failure. You write orders in plain text to four officers, up to three orders each day. A System One model, [TypeSafe Jev](https://typesafe.ai), measures each order one time. Each officer applies a different doctrine to the same measurement and selects one action. The selected actions share the fuel, the trucks, the crew hours and the medicine. The night runs. The morning report shows each action and its effects, with every number behind it.
...
1. **The judge** (`src/judge/questions.ts`). The browser builds a compact state for each order: the order text, the day, one line per active crisis, one line per resource, the active standing orders, the last four orders and any open officer question. The Worker sends the state and a fixed question set to Jev. The set has four Choices (objective, owner, sector, timeframe), forty Nouls (department scope, eleven priorities, twelve constraints, thirteen properties of the wording), six Scores (urgency, risk tolerance, resource flexibility, specificity, clarity, delegated discretion), two Nouls per active standing order and one Noul per open question. Jev returns probabilities in about 400 ms. It does not return a plan.
2. **The officers** (`src/engine/resolve.ts`, `src/content/officers.ts`, `src/content/actions.ts`). Each department has a finite action library. Each officer scores every action with a fixed formula. The formula adds the objective match, the stated priorities weighted by doctrine, the constraints the action honours or violates weighted by literalness, a penalty for a permission the order did not give, urgency times the action's speed, risk times risk aversion, the sector match, discretion, standing orders in scope, the department's record of past priorities and the officer's own initiative. The engine records every term.
3. **Clarification** (`src/engine/resolve.ts`). The engine calculates a need to ask from low clarity, an unnamed target, a contradiction, two resources with no precedence, and conflicts with a standing order or the record. Literalness raises the need. Initiative lowers it. If the need crosses the threshold, the officer asks an authored question and holds to routine until the answer arrives.
4. **The world** (`src/engine/world.ts`, `src/content/crises.ts`). Power output decides which sectors are heated. Cold in an unheated sector injures colonists and freezes the pipes in Works. Fuel feeds the generator, the pumps and the trucks. The pumps produce water. Medicine and a heated ward decide which patients live. Thirty-one crisis templates are gated by state and drawn by the seed. Every change to the world is an effect with a cause and one sentence.
5. **Memory** (`src/engine/orders.ts`, `src/engine/trust.ts`). An order that sets a rule becomes a standing order with its own measurement. Each order updates a per-department record of the priorities it stated. The record fades each night. Trust moves by fixed rules. Trust changes how much an officer reads intent, asks questions and follows the record.
...
| | doctrine | rule |
| --- | --- | --- |
| Captain Ilya, security | initiative .90, literalness .45, risk aversion .25 | Do not write "whatever it takes" unless you accept casualties. |
| Chen, logistics | literalness .92, resource caution 1.45, initiative .45 | State which resource comes first, or she asks and waits. |
...
`npm run calibrate` sends 46 authored orders through the real question set at a fixed day-six situation. The orders are clear, vague, contradictory, conditional, absolute, in conflict with a standing order, or addressed to the model. The script grades every answer against the engine's thresholds and writes [docs/CALIBRATION.md](docs/CALIBRATION.md). The report lists pass rates per question, the separation between orders labelled high and low, and the drift between repeated runs. If the corpus fails, change the question wording or the label. Do not change the thresholds to fit the wording.

`npm run sim` runs seeded games with scripted commanders and no model calls. It reports endings, deaths, clarification counts and action diversity. `scripts/morale.ts` sums every recorded morale effect and every death by its sentence. `scripts/play.ts` sends authored orders through the live judge, saves the event stream, and can replay any decision offline with `--explain <day> <department>`.
