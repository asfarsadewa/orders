...
export function explainMeasurements(m: Measurements): string[] {
  const out: string[] = [];
  out.push(`objective ${m.objective.choice} ${f2(m.objective.probabilities[m.objective.choice] ?? 0)} · target ${m.target.choice} ${f2(m.target.probabilities[m.target.choice] ?? 0)} · owner ${m.owner.choice} ${f2(m.owner.probabilities[m.owner.choice] ?? 0)} · sector ${m.sector.choice} ${f2(m.sector.probabilities[m.sector.choice] ?? 0)} · timeframe ${m.timeframe.choice}`);
  ...
  m.standing.forEach((s, i) => out.push(`standing ${s.standingOrderId ?? i + 1}: conflict ${f2(s.conflict)} · override ${f2(s.override)}`));
