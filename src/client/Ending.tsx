...
export function EndingScreen({ state, onReplay, onNew }: { state: GameState; onReplay(): void; onNew(): void }) {
  ...
  if (r.challengedMost) rows.push(["most objections", <>{OFFICERS[r.challengedMost.department].name}, {r.challengedMost.count} times</>]);
  ...
  if (r.clearButCostly.length) rows.push(["clear order before a costly night", <>{r.clearButCostly.slice(0, 2).map((o) => <q key={o.id}>{o.text}</q>)}</>]);
  if (r.sacrificed) rows.push(["resource with the largest loss", <>{r.sacrificed.name}, down {pct(r.sacrificed.fraction)}</>]);
