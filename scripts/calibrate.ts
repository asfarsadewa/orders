...
function grade(entry: CorpusEntry, m: Measurements): Check[] {
  ...
  oneOf(e.target, m.target.choice, m.target.probabilities, "target");
...
async function main(): Promise<void> {
  ...
  for (const r of first) {
    ...
    lines.push(`> ${r.entry.text}`);
    lines.push("");
    lines.push(`objective ${m.objective.choice} ${f2(m.objective.probabilities[m.objective.choice] ?? 0)} · target ${m.target.choice} ${f2(m.target.probabilities[m.target.choice] ?? 0)} · owner ${m.owner.choice} ${f2(m.owner.probabilities[m.owner.choice] ?? 0)} · sector ${m.sector.choice} ${f2(m.sector.probabilities[m.sector.choice] ?? 0)} · timeframe ${m.timeframe.choice} ${f2(m.timeframe.probabilities[m.timeframe.choice] ?? 0)}`);
    lines.push("");
